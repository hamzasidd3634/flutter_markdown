...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  final dynamic Function(bool)? onReadMore;
    ...
    if (_blocks.isNotEmpty && builders.containsKey(_blocks.last.tag)) {
      ...
      child = Scrollbar(
        child: SingleChildScrollView(
          ...
          child:
              _buildRichText(text: delegate.formatText(styleSheet, text.text)),
        ),
      );
    } else {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (builders.containsKey(tag)) {
        ...
        current.children.add(_buildRichText(text: const TextSpan(text: '\n')));
  ...
  Widget _buildRichText(
      {TextSpan? text, TextAlign? textAlign, String? key, Widget? texts}) {
